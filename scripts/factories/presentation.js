...
export default function presentationFactory() {
  function createPresentation(newPresentation) {
    const presentationInnerHTML = `<aside class="photographer__presentation" aria-label="informations">
        ...
        <p class="photographer__location" aria-describedby="${newPresentation.name}">${newPresentation.city},
         ${newPresentation.country}</p>
        <p class="photographer__quote" aria-describedbynewPresentation.name}">${newPresentation.tagline} </p>
        </aside>
        <button id="btnOppenModal" class="contact_button" 
        aria-label="contactez moi" aria-describedby="${newPresentation.name}">
            Contactez-moi
        </button>
        <img class="photographer__profile portrait" src="${newPresentation._pathPortrait}" 
        aria-label="${newPresentation.name}" alt="profil de ${newPresentation.name}" 
        aria-describedby="${newPresentation.name}">`;
