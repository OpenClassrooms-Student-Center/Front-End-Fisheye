...
*/
    function init () {
        const keysToListenTo = ['Enter', 13, 'ArrowDown', 40, 'ArrowUp', 38, 'Escape', 27, 'Tab', 9]
    ...
    */
        ...
        */
                ...
                customOptionsElement.setAttribute('aria-activedescendant', 'option-0')
            }
        ...
        customSelectedOptionElement.addEventListener('keydown', function (e) {
            const keyName = e.key || e.keyCode
            if (!keysToListenTo.includes(keyName)) return
            if (keyName === 'Enter' || keyName === 13) toggleFilterVisibility()
        })

        customOptionsElement.addEventListener('keydown', function (e) {
            const keyName = e.key || e.keyCode

            const stopFunction = !keysToListenTo.includes(keyName) || keyName === 'Enter' || keyName === 13
            if (stopFunction) return

            if (keyName === 'Escape' || keyName === 27) toggleFilterVisibility()

            const moveToOption = (keyName === 'ArrowDown') || (keyName === 40) || (keyName === 'ArrowUp') || (keyName === 38) || (keyName === 'Tab') || (keyName === 9)
            if (moveToOption) moveDownUpFromSelectedOption(e, keyName)
        })
    ...
    */
        function toggleFilterVisibility () {
            const closeFilter = !customOptionsElement.className.includes('custom-select__options--hidden')
            if (closeFilter) {
                customOptionsElement.classList.add('custom-select__options--hidden')
                customSelectedOptionElement.focus()
                customSelectedOptionElement.setAttribute('aria-expanded', 'false')
            } else {
                customOptionsElement.classList.remove('custom-select__options--hidden')
                customOptionsElement.focus()
                customOptionsElement.setAttribute('aria-activedescendant', 'option-0')
    ...
    */
        function moveFromOption (position) {
            ...
            customOptionsElement.setAttribute('aria-activedescendant', `option-${position}`)
        }
        ...
        Renvoie :
            - Rien
    */
        function moveDownUpFromSelectedOption (e, keyName) {
            e.stopPropagation()
            e.preventDefault()

            let position = 1; let elementToFocus
            if (keyName === 'ArrowDown' || keyName === 40 || (keyName === 'Tab') && !e.shiftKey) {
...
function createSelectedOptionElement (selectElement) {
    ...
    element.setAttribute('aria-controls', 'custom-options')
